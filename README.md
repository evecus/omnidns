...
```bash
# Linux amd64
curl -Lo omnidns https://github.com/evecus/omnidns/releases/latest/download/omnidns-linux-amd64
chmod +x omnidns
sudo mv omnidns /usr/local/bin/

# Linux arm64
curl -Lo omnidns https://github.com/evecus/omnidns/releases/latest/download/omnidns-linux-arm64
chmod +x omnidns
sudo mv omnidns /usr/local/bin/
```
...
```bash
curl -L https://github.com/evecus/omnidns/releases/latest/download/sha256sums.txt | sha256sum -c
```
...
```bash
sudo omnidns run -c /etc/omnidns/config.toml
```

### As an upstream for mihomo / sing-box

Run omnidns on a non-privileged port. No root required, no resolv.conf changes.
...
omnidns uses `.drs` (DNS Ruleset) binary files for routing rules. They are built from human-readable source files using the `omnidns build` command.
...
omnidns lookup gfw.drs google.com
# MATCH  DOMAIN        google.com

omnidns lookup gfw.drs sub.youtube.com
# MATCH  DOMAIN-SUFFIX  sub.youtube.com

omnidns lookup gfw.drs baidu.com
# NO MATCH              baidu.com
```
...
[[dns.rules]]
rulesets = ["/etc/omnidns/rules/ads.drs"]
action   = "reject"
...
omnidns build \
  --input cn.yaml \
  --format mihomo \
  --output /etc/omnidns/rules/cn.drs
...
omnidns build \
  --input adguard.txt \
  --format adguard \
  --output /etc/omnidns/rules/ads.drs
```
...
Run omnidns on a machine that acts as the default gateway. Intercept DNS from all LAN clients.
...
# Make permanent:
echo "net.ipv4.ip_forward=1" >> /etc/sysctl.d/99-omnidns.conf
```
...
```ini
# /etc/systemd/system/omnidns.service
[Unit]
Description=omnidns DNS proxy
After=network.target
Wants=network.target

[Service]
Type=simple
ExecStart=/usr/local/bin/omnidns run -c /etc/omnidns/config.toml
Restart=on-failure
RestartSec=5
...
sudo systemctl enable --now omnidns
sudo systemctl status omnidns
```
...
```
omnidns run    -c <config>                          Run the DNS proxy
omnidns build  --input <file> [--input <file>...]   Build a .drs ruleset
               --format <mihomo|adguard>
               --output <file>
omnidns lookup <ruleset.drs> <domain>               Test a domain against a ruleset
omnidns info   <ruleset.drs>                        Show ruleset metadata
```
...
```bash
git clone https://github.com/evecus/omnidns
cd omnidns
cargo build --release
# Binary at: target/release/omnidns
```
