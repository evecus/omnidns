...
pub struct UpstreamGroup {
    /// Server URLs: udp://, tcp://, tls://, https://, quic://, dhcp://iface, rcode://refused
    /// 不带前缀的 IP 字面量视为 udp://
            ...
            if has_domain_upstream {
                bail!(
                    "default-nameserver is required when manage-resolv-conf or firewall is enabled \
                     ...
                     Without it, resolving upstream domains would loop back to omnidns itself."
                );
            }
        }
...
fn default_v4_lease_file() -> String {
    "/var/lib/omnidns/dhcp4.leases".into()
}
fn default_v6_lease_file() -> String {
    "/var/lib/omnidns/dhcp6.leases".into()
}
