...
impl Config {
    /// 把规范化后的 Config 转回用户可读的 YAML（ConfigYaml 形状），写入磁盘。
    /// 用于配置热更新场景：表单编辑产生新的 Config → 序列化落盘。
    ...
    fn to_config_yaml(&self) -> ConfigYaml {
        let mut groups = self.groups.clone();
        let nameserver = groups
            .shift_remove("default")
            .unwrap_or(UpstreamGroup {
                servers: vec!["rcode://refused".to_string()],
                strategy: Strategy::default(),
                insecure: false,
                client_subnet: None,
            });

        let rules: IndexMap<RuleKey, UpstreamGroupSerde> = groups
            .into_iter()
            .map(|(name, g)| (RuleKey::Single(name), UpstreamGroupSerde::Full(g)))
            .collect();

        let rulesets = RulesetsSerde::List(self.rulesets.clone());
        ...
        ConfigYaml {
            ...
            default_nameserver: self.default_nameserver.iter().map(|a| a.to_string()).collect(),
            nameserver: UpstreamGroupSerde::Full(nameserver),
            rules,
            rulesets,
    ...
    /// 生成一份最小可用的空配置：DNS 部分留空占位（nameserver 指向
    /// rcode://refused，即"未配置"状态），web 面板强制开启，监听 0.0.0.0:8087，
    /// 以便用户首次运行时能立刻打开面板去完成配置。
    ...
    /// 加载配置；若文件不存在，则生成一份空配置并写入该路径，然后返回它。
    /// 用于 "relay run 不指定配置文件也能跑" 的需求。
    pub fn load_or_init(path: &std::path::Path) -> Result<Self> {
        if !path.exists() {
            let config = Self::generate_empty();
            config.save_to(path).with_context(|| {
                format!("Failed to write initial empty config to {}", path.display())
            })?;
            tracing::info!(
                "No config found at {}, generated an empty one. \
                 DNS resolution is disabled until configured via the web panel.",
                path.display()
            );
            return Ok(config);
        }
