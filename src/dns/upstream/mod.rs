...
fn parse_upstream_url(url: &str, insecure: bool) -> Result<UpstreamKind> {
    ...
    if let Some(rest) = url.strip_prefix("rcode://") {
        let code = match rest {
            "refused" => hickory_proto::op::ResponseCode::Refused,
            "nxdomain" => hickory_proto::op::ResponseCode::NXDomain,
            "servfail" => hickory_proto::op::ResponseCode::ServFail,
...
impl UpstreamGroup {
    ...
    async fn query_round_robin(&self, request: &Message) -> Result<Message> {
    ...
    async fn query_fastest(&self, request: &Message) -> Result<Message> {
