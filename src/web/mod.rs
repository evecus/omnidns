//! Web 面板：axum HTTP 服务 + 静态资源 + Session Auth + JSON API。
//!
//! 路由树：
//!   /api/auth/*        登录/登出/改密码（登录/登出不需要鉴权，改密码需要）
//!   /api/*             其余 JSON API（若 auth.enable 则挂 session 鉴权中间件）
...
pub async fn serve(
    ...
    // 鉴权相关路由：login/logout 不需要鉴权中间件本身
    // （否则没登录就永远进不了 login），password 修改需要已登录。
    let auth_routes = Router::new()
        .route("/login", post(api::auth_login))
        .route("/logout", post(api::auth_logout))
        .with_state(api_state.clone());

    let auth_protected_routes = Router::new()
        .route("/password", post(api::auth_change_password))
    ...
    // 业务 API 路由（需要鉴权，除非 auth.enable=false）
